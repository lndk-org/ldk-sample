...
fn handle_ldk_events<'a>(
	...
	bump_tx_event_handler: &'a BumpTxEventHandler, peer_manager: Arc<PeerManagerType>,
					...
					for address in addresses {
						if let Ok(sockaddrs) = address.to_socket_addrs() {
							for addr in sockaddrs {
								...
								if node_api::connect_peer_if_necessary(node_id, addr, pm)
									.await
									.is_ok()
								{
									return;
...
pub async fn start_ldk(args: config::LdkUserInfo, test_name: &str) -> node_api::Node {
	...
	let logger =
		Arc::new(FilesystemLogger::new(ldk_log_dir.clone(), args.log_level, args.node_num));
	...
	let background_processor = tokio::spawn(process_events_async_with_kv_store_sync(
		Arc::clone(&persister),
		event_handler,
		Arc::clone(&chain_monitor),
		Arc::clone(&channel_manager),
		Some(onion_messenger.clone()),
